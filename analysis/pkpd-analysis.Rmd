...
pwh_simulx_base =  filter(pwh_pkpd_simulx, model == "PWH PKPD")
...
# this runs a series regression to find shallowest curve (approx HL)
# dat is the popPK mean curve for a given group
fit_lm_df = function(dat, day_min = 3, day_max = 28, min_window = 7, lloq_ug = 0.2){

  map_df(day_min:(day_max - min_window), function(dlow){
   map((dlow+7):day_max, function(dhigh){
      lm_dat_sub = filter(dat, day >= dlow & day <= dhigh)
      # lloq creates artificially shallow curves
      if(any(lm_dat_sub$L == lloq_ug)) return(NULL)
      tibble(
        day_start = dlow,
        day_end = dhigh,
        mod = list(lm(logL ~ day, data = lm_dat_sub))
      )
    }) %>%
    list_rbind()
  }) 
...
approx_hl_setup = bind_rows(pwh_simulx_base, pwh_simulx_covadj, pwoh_simulx_final) %>%
  mutate(
    model_table = case_when(
      str_detect(model, "PWOH") ~ "PWOH",
      str_detect(model, "ADJ") ~ "PWH (Covariate-Adjusted)",
      T ~ "PWH (Base)"
    )
  ) %>%
  dplyr::select(model_table, pop_mean_conc) %>%
  unnest(pop_mean_conc) %>%
  filter(str_detect(model_table, "PWH") | group %in% c("E", "F")) %>%
  mutate(L = pmax(L, 0.2), logL = log(L), day = time) %>%
  group_by(model_table, group, dose) %>%
  nest() %>%
  mutate(
    approx_hl_df = map(data, fit_lm_df)
  ) 

approx_hl = approx_hl_setup %>%
  unnest(approx_hl_df) %>%
  mutate(
    hl = map_dbl(mod, ~log(0.5)/coef(.)[['day']]),
    maxhl = hl == max(hl)
  ) %>%
  filter(maxhl) %>%
  mutate(
    dose_full = paste(dose, "mg/kg"),
    pred = map(mod, ~(tibble(day = 0:21, 
                             logL = predict(., newdata = tibble(day = day)),
                             L = exp(logL)))
    )
  ) %>%
  ungroup() %>%
  mutate(model_plot = fct_relevel(factor(model_table), "PWOH"))


max_hl = bind_rows(mutate(pwoh_pk_parms, model_table = "PWOH"), pwh_pkpd_parms) %>%
  select(model_table, parameter, value) %>%
  spread(parameter, value) %>%
  group_by(model_table) %>%
  summarize(
    hl = clinPK::pk_2cmt_t12(CL = Cl_pop, V = V_pop, 
                             Q = Q_pop, V2 = V2_pop)$beta,
    .groups = "drop"
  ) %>%
  mutate(
    table_label = "Maximum half-life"
  )


hl_table = approx_hl %>%
  mutate(
    table_label = paste("Approx. half-life at",
                        dose_full)
    
  ) %>%
  select(model_table, table_label, hl) %>%
  bind_rows(max_hl)

# spread(hl_table, model_table, hl)
...
## Table 2: PK Parm Tables

Manuscript table assembled in two parts

```{r hl-table}


hl_table %>%
  pivot_wider(names_from = "model_table", values_from = "hl") %>%
  relocate(table_label, PWOH) %>%
  rename(`HL computation (days)` = table_label) %>%
    kable(
      digits = 1,
      format = output_type,
      caption = "Half-life calculations",
      escape = FALSE,
      booktabs = TRUE
    ) %>%
...
```{r cd8-pl, fig.height=6.5, fig.cap = "CD8 levels"}
cd8_pls = map(c("cd8_pct", "cd8_count"), function(i){
  ytitle = switch(i, 
                  cd8_pct  = "CD8 percent",
                  cd8_count = "CD8 Count (cells/µL)")
  ylims = switch(i, 
                  cd8_pct  = c(0, 75),
                  cd8_count = c(0, 1600))
...
```{r all-cds,  fig.height=6.5, fig.cap = "CD4 and CD8 levels"}

cd_pls_list = list(cd4_pls[[1]], cd8_pls[[1]], cd4_pls[[2]], cd8_pls[[2]])

cd_pls_list = map(cd_pls_list, ~(..1 + theme(strip.text = element_text(size = 7))))

patchwork::wrap_plots(cd_pls_list, nrow = 1) + plot_layout(axis_titles = "collect")
...
```{r approx-hl-pl, fig.scap = "Approx half-life estimation.", fig.cap = "Estimation. of approximate half-life by finding slowest linear phase (dashed red line) overlaid on serum concentration curve. Population serum concentration curves computed based on the population PK parameters. Approximate half-life computed for high dose groups (10 mg/kg and 30 mg/kg) for each cohort (PWH and PWOH). No covariate-adjusted model was fit for PWOH. Vertical dotted lines indicate range (minimum 1 week) of data where longest half-life estimated."}

approx_hl %>%
  unnest(data) %>%
  filter(day < 35) %>%
  ggplot(aes(x = day, y = L)) +
  geom_line(linewidth = 1) +
  geom_line(
    data = unnest(approx_hl, pred),
    color = "red",
    linewidth = 1,
    linetype = 5
  ) +
  geom_vline(aes(xintercept = day_start),  color = "red", linetype = "dotted") +
  geom_vline(aes(xintercept = day_end),  color = "red", linetype = "dotted") +
  geom_text(data = mutate(approx_hl, 
                          yloc = map2_dbl(mod, day_end, ~predict(..1, tibble(day = ..2)))), 
            aes(label = paste("HL =", round(hl, 1), "days"), 
                x = day_end, y = exp(yloc)),
            hjust = 0, vjust = 0) +
  scale_y_log10(
    breaks = c(.2, 1, 10, 100, 1000),
    labels = c(as.expression(bquote("" <= .(
      0.2
    ))), 1, 10, 100, 1000),
    limits = c(0.2, 5000)
  ) +
  facet_grid(cols = vars(model_plot), rows = vars(dose_full)) +
  labs(y = conc_title, x = "Days post-administration")
